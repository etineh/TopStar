package com.pixel.chatapp.view_controller.calls;
...
public class CallCenterActivity extends AppCompatActivity implements CallRepository.Listener, CallsListener {
...
//        });
        ...
        callRepository.initialiseWebRTC(myUserName, getApplicationContext(), otherUid, otherName, myId);

        callRepository.listener = this;

        callRepository.initLocalView(localViewSmallFrame);
        callRepository.initRemoteView(remoteViewBigFrame);  // connect other user video
        ...
        if(callType != null) {
            if(callType.equals("audio")){
                ...
                callRepository.toggleVideo(false);  // off camera
                isCameraMuted= true;
        ...
        View.OnClickListener acceptVideo = v -> {
            ...
            callRepository.toggleVideo(true);  // turn on camera!
            isCameraMuted= false;
    ...
    private void answerCallOrSendCallRequest(DataModelType type){
        ...
        if(answerCall){
            callRepository.answerCall(otherUid);     // answer the call
...
//                        Toast.makeText(CallCenterActivity.this, "I end call none", Toast.LENGTH_SHORT).show();
    ...
    private void callEnd(Boolean onAnotherCall){
        ...
        callRepository.endCall();   // close all camera settings
...
//        runOnUiThread(this::finish);
    ...
    public void acceptVideoCall() { // notify me when user accept my video call
        ...
        callRepository.toggleVideo(true);  // turn on!
        isCameraMuted= false;
